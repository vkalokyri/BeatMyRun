...
public class StudentRepo {
    ...
    public Student compareTime(SQLiteDatabase db)
    {
        ...
        String compareQuery =  "SELECT " +
                ...
                " FROM " + Student.TABLE
                + " WHERE "+Student.KEY_age+" = (SELECT MIN("+Student.KEY_age+") FROM " +Student.TABLE+ ")";// It's a good practice to use parameter ?, instead of concatenate string

        int iCount =0;
        Student student = new Student();

        Cursor cursor = db.rawQuery(compareQuery, new String[] { } );
